...
from colossalai.fx.tracer._tracer_utils import extract_meta
...
class ColoTracer(Tracer):
    ...
    def create_proxy(self,
            ...
            try:
                if target == '__call__':
                    ...
                    if target not in _TensorPropertyMethod:
                        proxy.meta_data = getattr(unwrap_fn(args[0]), target)(*tree_map(unwrap_fn, args[1:]),
        ...
        # print(kind, name, target, type(target))
        return proxy
...
def symbolic_trace(
    ...
    if is_compatible_with_meta():
        ...
        from colossalai.fx.tracer import ColoTracer as OrigColoTracer
...
def bias_addition_pass(gm: ColoGraphModule, root_model: torch.nn.Module, meta_args: Optional[Dict[str, Any]]=None):
    ...
    value_remap = {}
    unwrap_fn = lambda n: n._meta_data if isinstance(n, Node) else n
    # wrap_fn = lambda n: ColoProxy(n, tracer) if isinstance(n, Node) else n

    for ori_node in gm.graph.nodes:
        assert hasattr(ori_node, "_meta_data")
        kind = ori_node.op
